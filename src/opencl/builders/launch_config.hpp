...
void validate_grid_dimension_compatibility(device_t const&device, nd_range::workgroup_dimensions_t workgroup_dims) noexcept(false);
// void validate_compatibility(const context::device::handle_t, memory::shared::size_t, bool) noexcept(false);
...
// using opencl::nd_range::dimension_t;
// using opencl::nd_range::overall_dimension_t;
...
public:
	...
	using offset_t = nd_range::offset_t;
...
protected:
	...
	/// Use the information specified for the builder to figure out the dimension information
	/// necessary for the kernel launch
	...
	optional<offset_t> get_offset() const noexcept(false)
	{
#ifndef NDEBUG
...
public:
	...
	void validate_offset(offset_t const& offset) const
	{
	...
	launch_configuration_t& num_workgroups(size_t size) { return grid_size(size); }
	///@}
	...
	// template<
	// 	typename... Ts,
	// 	typename = opencl::detail::all_true<std::is_same<dimension_t, Ts>::value ...>>
	// launch_configuration_t& overall_dimensions(Ts&&... dims)
	// {
	// 	return overall_dimensions(dimensions_t{dims...});
	// }

	launch_configuration_t& overall_dimensions(
		dimension_t x,
		dimension_t y = 1,
		dimension_t z = 1)
	{
	...
	launch_configuration_t& offset(optional<nd_range::offset_t> const& offset)
	{
		offset_ = offset;
#ifndef NDEBUG
	...
	launch_configuration_t& clear_offset() { return offset(nullopt); }
	launch_configuration_t& no_offset() { return clear_offset(); }

	launch_configuration_t& round_up_overall_dims() { round_up_overall_dims_ = true; return *this; }
	launch_configuration_t& dont_round_up_overall_dims() { round_up_overall_dims_ = false; return *this; }
