...
int main(int argc, char** argv)
{
    ...
    auto get_ith_platform = [] (char const* i_str) {
        auto i = std::strtol(i_str, nullptr, 10);
        return opencl::platform::get(i); // note i is 0-based
    };

    auto platform = (argc > 1) ? get_ith_platform(argv[1]) : get_first_opencl_2_platform();
    std::cout << "Using OpenCL platform " << platform.index() << " (" << platform.name() << ")\n";
    ...
    write_only pipe int outPipe)
{
  ...
  output[get_global_size(0)+get_global_id(0)] =
        inputA[get_global_size(0)+get_global_id(0)]
      + inputB[get_global_size(0)+get_global_id(0)]
      + globalA;
}
)";

    auto source = opencl::program::source::create(context, {"trivialSizeTCompat.cl", program_source});
    auto options = opencl::program::compilation::options::create();
    options.language_version = opencl::make_version("2.0");
    std::cout << "Compilation command-line options: " << options.render() << '\n';
    auto build_result = opencl::program::build_(source, device, options);
    ...
    auto pipe = [&] {
        auto packet_size = sizeof(cl_int);
        auto capacity = numElements/2;
        return opencl::pipe::create(context, packet_size, capacity);
    }(); // TODO: Offer a pipe builder
    ...
    queue.enqueue_kernel_launch(vectorAdd, launch_config,
        input_a_buffer, input_b_buffer, output_buffer, pipe, 3);
    queue.enqueue_kernel_launch(vectorAdd, launch_config,
        input_a_buffer, input_b_buffer, output_buffer, pipe, 3);
    ...
    std::cout << "Max pipe args: " << caps.get_raw_parameter<CL_DEVICE_MAX_PIPE_ARGS>() << '\n';
    std::cout << "Max pipe active reservations: " << caps.get_raw_parameter<CL_DEVICE_PIPE_MAX_ACTIVE_RESERVATIONS>() << '\n';
    std::cout << "Max pipe packet size: " << caps.get_raw_parameter<CL_DEVICE_PIPE_MAX_PACKET_SIZE>() << '\n';


    std::cout << "Output (in hexadecimal):\n";
    std::cout << std::hex;
